...
class ParallelImageDownloader:
    ...
    def generate_image_urls(self, image_id: str, highres_only: bool = True) -> List[Tuple[str, str, str]]:
        """
        画像IDから複数の解像度のURLを生成
        
        Args:
            image_id (str): 画像ID（例: ISS008-E-19646）
            highres_only (bool): 高解像度のみを取得するか
            
        Returns:
            List[Tuple[str, str, str]]: (URL, ファイル名, 解像度) のリスト
        """
        urls = []
        ...
        if highres_only:
            # 高解像度のみを取得
            url_patterns = [
                (f"https://eol.jsc.nasa.gov/DatabaseImages/EFS/highres/{mission}/{image_id}.JPG", "highres", "高解像度")
            ]
        else:
            # 異なる解像度のURLパターン（正しいディレクトリ構造）
            url_patterns = [
                (f"https://eol.jsc.nasa.gov/DatabaseImages/ESC/small/{mission}/{image_id}.JPG", "small", "小"),
                (f"https://eol.jsc.nasa.gov/DatabaseImages/ESC/large/{mission}/{image_id}.JPG", "large", "大"),
                (f"https://eol.jsc.nasa.gov/DatabaseImages/EFS/highres/{mission}/{image_id}.JPG", "highres", "高解像度")
            ]
        
        for url, size, description in url_patterns:
            filename = f"{image_id}.JPG"  # ディレクトリ分けしないので、シンプルなファイル名
            urls.append((url, filename, description))
    ...
    def select_best_image(self, image_id: str, downloaded_files: List[Dict]) -> Optional[Dict]:
        """
        複数解像度から最も解像度の高い画像を選択
        ...
        if self.debug:
            print(f"🎯 {image_id}: 最適解像度選択 - {best_file['resolution']} ({best_file.get('file_size', 0):,} バイト)")
    ...
    def download_images_parallel(self, json_file: str, base_dir: str = "downloads", highres_only: bool = False) -> Dict:
        """
        並列で画像をダウンロード
        ...
        for image_id, metadata in image_data.items():
            # 全解像度を取得
            urls = self.generate_image_urls(image_id, highres_only)
        ...
        for image_id, downloads in image_downloads.items():
            ...
            if successful_downloads:
                # 最適な解像度を選択
                best_download = self.select_best_image(image_id, successful_downloads)
                if best_download:
                    # 最終的なファイル名にリネーム
                    ...
                    try:
                        # ファイルをリネーム
                        os.rename(best_download['filepath'], final_filepath)
                        
                        # 他の解像度のファイルを削除
                        for download in downloads:
                            if download['filepath'] != best_download['filepath'] and os.path.exists(download['filepath']):
                                os.remove(download['filepath'])
                        
                        results['successful'].append({
                            'image_id': image_id,
                            'resolution': best_download['resolution'],
                            'filepath': final_filepath,
                            'message': f"最適解像度選択: {best_download['resolution']}",
                            'info': best_download['info']
                        })
