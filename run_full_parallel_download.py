...
earth_observatory_ids.jsonを使用して30並列で高解像度画像のみダウンロード
"""
...
def main():
    ...
    # 並列ダウンロード実行（全解像度から最適解像度を選択）
    print(f"\n🚀 並列ダウンロード開始（全解像度から最適解像度を選択）...")
    results = downloader.download_images_parallel(json_file, "downloads", highres_only=False)
